    ...
    public static class IoHelper
    {
        ...
        private static bool ShouldExclude(string path)
        {
            ...
            var shouldExclude =
                segments.Any(
                    directory =>
                        AppConfiguration.GetDirectoryExclusions().Any(exclusion => Regex.IsMatch(directory, exclusion)));

            if (!shouldExclude)
            {
                var file = segments.Last();
                shouldExclude = AppConfiguration.GetFileExclusions().Any(exclusion => Regex.IsMatch(file, exclusion));
            }
